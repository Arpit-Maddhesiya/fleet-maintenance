...
Each module's prompt is pasted in full here as the frontend work happens, with the same structure: full prompt text, what I got, what I corrected.
...
- shadcn/ui initialized (radix, neutral, new-york style) with button, input, label, table, dialog, dropdown-menu, badge, card, select, tabs, sonner. Tailwind v4 theme (`@theme inline` + oklch tokens) written into `globals.css`, `cn()` in `lib/utils.ts`, Geist fonts + `ThemeProvider` + `Toaster` in the root layout.
- Login page reworked: server component + client form using `useActionState`; `loginAction` returns `{ error }` instead of redirecting silently, and the form renders a visible destructive alert on bad credentials.
- Authenticated shell `(app)/layout.tsx`: server component that redirects logged-out users to `/login` (the proxy does this too, but the layout is the second line), wraps children in `SessionProvider`, and renders a sidebar `AppNav`.
- `AppNav`: role-aware links (Dashboard, Vehicles, Service Records for everyone; My Records only for technicians; Alerts with a live count badge fetched from `/api/alerts` and refetched on route change), sign-out button. Commented clearly that hiding manager-only links is cosmetic — the backend's `requireRole()` is the real boundary.
- `lib/api-client.ts`: `apiFetch<T>()` adds `credentials: "include"`, serializes JSON (or sends FormData for the bulk upload), parses the response, and throws a typed `ApiError` carrying the server's `{ error }` message on non-2xx.
- Placeholder pages for every nav destination.

Verified: `next build` compiles and typechecks, all routes registered; dev server serves `/login` (200) and redirects unauthenticated `/dashboard` to `/login` (307); full Vitest suite still 59/59.

#### What I corrected

- The shadcn CLI (4.19) changed its flags: `-b` is now the component library (`base`/`radix`/`aria`), not the base color, and it auto-detects the framework — which failed on this project layout. I created `components.json` manually and used `shadcn add` per component. The CLI also did **not** install `lucide-react`, `class-variance-authority`, or create `lib/utils.ts` / the Tailwind theme — all of which the generated components import. I installed the missing deps and wrote the theme + `cn()` by hand.
- The original login action swallowed the `AuthError` and returned nothing, which made bad credentials look like a silent no-op. Changed it to return an `{ error }` state the form displays.
- The module says Next.js 15 / `middleware.ts`, but this project is Next.js 16 — the file is `src/proxy.ts` exporting `proxy()`. Kept the existing file; no "fix" needed.
...
- `lib/types.ts` — shared client types mirroring the backend JSON shapes (`Vehicle`, `ServiceRecord`, `VehicleWithRecords`, create/update inputs).
- `lib/vehicle-status.ts` — display-side status helper. The backend does NOT expose a per-vehicle due status (dashboard only gives counts), so I computed it from the vehicle + its records: a DUE record past the 7-day grace → OVERDUE (reusing the backend's exact definition from `lib/overdue.ts`), an elapsed date-or-mileage interval → DUE, else OK. Commented that it must stay in sync with `lib/overdue.ts`.
- Vehicles list page — table (registration links to detail, make/model, odometer, date interval, mileage interval, Active/Archived badge), "Show archived" toggle, manager-only "Add Vehicle" button, loading skeleton, real empty states (including "no archived"), and a fetch-error banner with retry.
- `VehicleDialog` — create + edit in one dialog. Client validation mirrors the backend Zod rules exactly with a "must stay in sync" comment (sharing the backend schema would couple the client to the server's Prisma import chain, so the module's fallback was used). Disables the registration number on edit and hides the odometer field on edit (PATCH doesn't accept it). Toasts on success/failure, refreshes the list.
- `ArchiveAction` — confirm-before-archive dialog (destructive button, clear warning that history is kept); restore is immediate.
- Vehicle detail page — core info cards, status badge (Overdue/Due/OK + Archived), Edit dialog (manager-only), and the service-history table (description, status badge, created, completed) with an empty state. The history row is a simple inline stub; F2 builds the real reusable row.

Verified: `next build` typechecks; `/vehicles` and `/vehicles/[id]` both 307-redirect unauthenticated visitors to `/login`; full Vitest suite still 59/59.

#### What I corrected

- My first version of the status helper had an unreachable OVERDUE branch (computed interval due but never the grace-period rule). Rewrote it to check the vehicle's active DUE record against the grace period first — matching the backend's `isOverdue()` — before falling back to the interval check.
- The detail page initially had an Edit button linking to `/vehicles/[id]/edit` (a page that doesn't exist — the module wants a dialog). Replaced it with the shared `VehicleDialog`.
